...
public class ReceptionistView extends JPanel{
    ...
    static String ReceptionistFile = "src/main/resources/receptionist.txt";
    static String AccountFile = "src/main/resources/users.txt";
    Font buttonFont;

    public ReceptionistView(AdminView parent){
        super();
        ...
        titleLabel=new JLabel("Receptionist List",SwingConstants.CENTER);
        ...
        String[] columnName={"Username","Password","Full Name","Date of Birth","Age","Gender","Contact",
                "Email","Race","IC/Passport","Address","Salary","Working Shift","Work Status"};
        ...
        //read file from registration
        //read from user.txt first
        Map<String,String> usernamePasswordMap=new LinkedHashMap<>(); //control the sequence
        java.util.List<String> user= Registration.readFile(AccountFile);
        for (String line:user){
            String[] parts = line.split(";");
            if (parts.length >=3 &&parts[2].equalsIgnoreCase("receptionist")){
                usernamePasswordMap.put(parts[0],parts[1]); //put the username and password in a map
            }
        }
        //second read from receptionist.txt
        List<String> tutor=Registration.readFile(ReceptionistFile);
        ...
        deleteButton.addActionListener(e->{
            ...
            int option =JOptionPane.showConfirmDialog(
                    parentWindow,"Are you sure want to delete Receptionist \""+fullName+"\"(Username:"+username+")?",
                    "Confirm Deletion",JOptionPane.YES_NO_OPTION);

            if (option==JOptionPane.YES_OPTION){
                String result=Registration.DeleteReceptionist(username);
                if (result.equals("Success")){
                    JOptionPane.showMessageDialog(this, "Receptionist "+username+" delete successfully.");
                    ((DefaultTableModel)table.getModel()).removeRow(SelectedRow); //update the GUI of the table
                    parent.refreshCount();
                }else{
                    JOptionPane.showMessageDialog(this, "Failed to delete Receptionist: " + result);
                }
            }
        });
    ...
    public void refreshTable(){
        ...
        model.setRowCount(0);
        List<Receptionist> receptionists = Registration.getAllReceptionist();
        ...
        for (Receptionist receptionist:receptionists){
            Object[] rowData={ //every message become an object list
                    ...
                    receptionist.getRace(),
                    receptionist.getIC(),
                    receptionist.getAddress(),
                    receptionist.getSalary(),
                    receptionist.getWorkingShift(),
